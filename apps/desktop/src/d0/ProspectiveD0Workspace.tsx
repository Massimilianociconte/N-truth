...
import { PseudoreplicationRiskCard } from "./PseudoreplicationRiskCard";
import { countUniqueAllocationUnits, createSampleRow, D0RowIdentity, sampleSheetLocator } from "./rowHelpers";
import { canonicalIssues, draftTarget, fieldId, validateD0, type D0FieldTarget, type D0ValidationIssue } from "./validation";
  ...
  return groups.flatMap((group) =>
    phases.map((phase) => {
      ...
      return {
        ...
        sourceCell: sampleSheetLocator(rows.length),
...
export function ProspectiveD0Workspace({
  ...
  const [rowIdentity] = useState(() => new D0RowIdentity());
  const [compileIssues, setCompileIssues] = useState<D0ValidationIssue[]>([]);
  const [focusTarget, setFocusTarget] = useState<D0FieldTarget | null>(null);

  useLayoutEffect(() => {
    if (focusTarget) document.getElementById(fieldId(focusTarget))?.focus();
  }, [focusTarget]);

  const navigateToField = (target: D0FieldTarget) => {
    setStep(target.step);
  ...
  const localIssues = useMemo(() => validateD0(draft, rows, language), [draft, rows, language]);
  const missingFacts = localIssues.filter((issue) => issue.category === "required");
  const invariantErrors = localIssues.filter((issue) => issue.category === "invariant");
  const capabilityErrors = localIssues.filter((issue) => issue.category === "capability");
  const allIssues = [...localIssues, ...compileIssues];
  const fieldProps = (target: D0FieldTarget) => {
    const id = fieldId(target);
    const issues = allIssues.filter((issue) => issue.targets.some((item) => fieldId(item) === id));
    return { id, "aria-invalid": issues.some((issue) => issue.severity === "error") || undefined,
      "aria-describedby": issues.map((issue) => issue.id).join(" ") || undefined };
  };
  const draftProps = (field: keyof D0DesignDraft) => fieldProps(draftTarget(field));
  const rowProps = (row: number, field: keyof SampleSheetRow) => fieldProps({ step: "samples", row, field });
  ...
  const evidence = useMemo<D0Evidence[]>(
    () => [
      ...
      },
      {
        ...
        locator: sampleSheetLocator(rows.length),
        text: text(
          language,
  ...
  const updateDraft = <Field extends keyof D0DesignDraft>(
    ...
    if (field === "levelA" || field === "levelB") {
      ...
      setRows((current) =>
        current.map((row) =>
          row.factorLevel === previous ? rowIdentity.update(row, { factorLevel: String(value) }) : row,
        ),
      );
    }
    if (field === "endpointId") {
      setRows((current) => current.map((row) => rowIdentity.update(row, { endpointId: String(value) })));
    }
  ...
  const updateRow = <Field extends keyof SampleSheetRow>(
    ...
    setRows((current) =>
      current.map((row, rowIndex) => (rowIndex === index ? rowIdentity.update(row, { [field]: value }) : row)),
    );
    resetCompilation();
  };

  const addRow = () => {
    setRows((current) => [...current, createSampleRow(current, draft.endpointId)]);
    resetCompilation();
  };
  ...
  const compileCanonical = async () => {
    ...
    const payload: ProspectiveD0CompilePayload = {
      ...
      language,
      rulesetId: D0_RULESET_ID,
      rulesetVersion: D0_RULESET_VERSION,
    };
    try {
      const result = await compileProspectiveD0(payload);
      if (compileGeneration.current === generation) {
        setCompileResult(result);
        setCompileIssues(canonicalIssues(result.issues, rows));
      ...
      if (error instanceof ApiError) {
        ...
        setCompileIssues(canonicalIssues(typeof detail === "object" && detail && "issues" in detail ? detail.issues : [], rows));
        const detailCode =
          typeof detail === "object" && detail && "code" in detail
            ? String(detail.code)
            : "";
        setCompileError(
          [detailCode || error.message, `[HTTP ${error.status}]`]
            .filter(Boolean)
            .join(" · "),
        );
      } else {
    ...
    >
      ...
      <div className="d0-layout">
        <div className="d0-step-content">
          {localIssues.length > 0 && <div className="d0-validation-list" aria-label={text(language, "Errori del draft D0", "D0 draft errors")}>
            <div><strong>{text(language, "Fatti, invarianti o limiti di profilo da risolvere", "Facts, invariants, or profile limits to resolve")}</strong>
              <ValidationSummary issues={localIssues} language={language} onNavigate={navigateToField} />
            </div>
          </div>}
          {(compileError || compileIssues.length > 0) && <div className="d0-validation-list" role={compileError ? "alert" : undefined}>
            <div><strong>{compileError ? text(language, "Compilazione canonica non completata", "Canonical compilation did not complete") : text(language, "Issue canoniche", "Canonical issues")}</strong>
              {compileError && <p>{compileError}</p>}
              <ValidationSummary issues={compileIssues} language={language} onNavigate={navigateToField} />
            </div>
          </div>}
          {step === "scope" && (
            <div className="d0-form-section">
              ...
              <div className="d0-field-grid">
                <label className="field-label d0-wide">
                  <span>{text(language, "Domanda primaria", "Primary question")} <span aria-hidden="true">*</span></span>
                  <textarea {...draftProps("question")} aria-required="true" value={draft.question} rows={2} onChange={(event) => updateDraft("question", event.target.value)} />
                </label>
                <label className="field-label d0-wide">
                  <span>{text(language, "Target inferenziale", "Inference target")} <span aria-hidden="true">*</span></span>
                  <input {...draftProps("inferenceTarget")} aria-required="true" value={draft.inferenceTarget} onChange={(event) => updateDraft("inferenceTarget", event.target.value)} />
            ...
            <div className="d0-form-section">
              ...
              <div className="d0-field-grid">
                <label className="field-label">
                  ...
                  <input {...draftProps("factorName")} aria-label={text(language, "Fattore primario", "Primary factor")} value={draft.factorName} onChange={(event) => updateDraft("factorName", event.target.value)} />
                </label>
                <label className="field-label">
                  factor_kind *
                  <select {...draftProps("factorKind")} aria-label="factor_kind" value={draft.factorKind} onChange={(event) => updateDraft("factorKind", event.target.value as D0DesignDraft["factorKind"])}>
                ...
                <label className="field-label">
                  ...
                  <input {...draftProps("levelA")} value={draft.levelA} onChange={(event) => updateDraft("levelA", event.target.value)} />
                </label>
                <label className="field-label">
                  {text(language, "Livello B", "Level B")} *
                  <input {...draftProps("levelB")} value={draft.levelB} onChange={(event) => updateDraft("levelB", event.target.value)} />
                </label>
                <label className="field-label">
                  allocation_level *
                  <select {...draftProps("allocationLevel")} aria-label="allocation_level" value={draft.allocationLevel} onChange={(event) => updateDraft("allocationLevel", event.target.value as D0DesignDraft["allocationLevel"])}>
                    <option value="Well">Well</option><option value="Plate">Plate</option><option value="UNKNOWN">UNKNOWN</option>
                  </select>
                </label>
                <label className="field-label">
                  application_level
                  <select {...draftProps("applicationLevel")} aria-label="application_level" value={draft.applicationLevel} onChange={(event) => updateDraft("applicationLevel", event.target.value as D0DesignDraft["applicationLevel"])}>
                    <option value="Well">Well</option><option value="Plate">Plate</option><option value="UNKNOWN">UNKNOWN</option>
                  </select>
                </label>
                <label className="field-label">
                  independently_assigned *
                  <select {...draftProps("independentlyAssigned")} aria-label={text(language, "Indipendenza dell'assegnazione", "Assignment independence")} value={draft.independentlyAssigned} onChange={(event) => updateDraft("independentlyAssigned", event.target.value as D0DesignDraft["independentlyAssigned"])}>
                ...
                <label className="field-label">
                  ...
                  <textarea {...draftProps("independenceMechanism")} value={draft.independenceMechanism} rows={2} onChange={(event) => updateDraft("independenceMechanism", event.target.value)} />
                </label>
                <label className="field-label d0-wide">
                  shared_environment
                  <input {...draftProps("sharedEnvironment")} value={draft.sharedEnvironment} onChange={(event) => updateDraft("sharedEnvironment", event.target.value)} />
                </label>
                <label className="field-label">
                  {text(language, "Endpoint primario", "Primary endpoint")} *
                  <input {...draftProps("endpointName")} value={draft.endpointName} onChange={(event) => updateDraft("endpointName", event.target.value)} />
                </label>
                <label className="field-label">
                  endpoint_id *
                  <input {...draftProps("endpointId")} value={draft.endpointId} onChange={(event) => updateDraft("endpointId", event.target.value)} />
                </label>
                ...
                <label className="field-label">
                  ...
                  <select {...draftProps("targetBiologicalUnit")} aria-label="target_biological_unit" value={draft.targetBiologicalUnit} onChange={(event) => updateDraft("targetBiologicalUnit", event.target.value as D0DesignDraft["targetBiologicalUnit"])}>
                    <option value="Well">Well</option><option value="Plate">Plate</option>
                  </select>
                </label>
                <label className="field-label">
                  effect_measure *
                  <input {...draftProps("effectMeasure")} aria-label="effect_measure" value={draft.effectMeasure} onChange={(event) => updateDraft("effectMeasure", event.target.value)} />
                </label>
                <label className="field-label d0-wide">
                  target_population_or_unit *
                  <input {...draftProps("estimandTargetPopulation")} aria-label="target_population_or_unit" value={draft.estimandTargetPopulation} onChange={(event) => updateDraft("estimandTargetPopulation", event.target.value)} />
                </label>
                <label className="field-label">
                  generalization_level *
                  <input {...draftProps("generalizationLevel")} aria-label="generalization_level" value={draft.generalizationLevel} onChange={(event) => updateDraft("generalizationLevel", event.target.value)} />
                </label>
                <label className="field-label">
                  estimand_condition
                  <input {...draftProps("estimandCondition")} aria-label="estimand_condition" value={draft.estimandCondition} onChange={(event) => updateDraft("estimandCondition", event.target.value)} />
                </label>
                <label className="field-label">
                  reviewer_role *
                  <input {...draftProps("reviewerRole")} aria-label="reviewer_role" value={draft.reviewerRole} onChange={(event) => updateDraft("reviewerRole", event.target.value)} />
                </label>
              </div>
            </div>
          )}
            ...
            <div className="d0-form-section">
              ...
              <div className="d0-table-wrap">
                <table className="d0-sample-table" aria-label="SampleSheet D0">
                  ...
                  <tbody>
                    {rows.map((row, index) => (
                      <tr key={rowIdentity.key(row)}>
                        <td><input {...rowProps(index, "sampleId")} aria-label={`sample_id ${index + 1}`} value={row.sampleId} onChange={(event) => updateRow(index, "sampleId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "sourceId")} aria-label={`source_id ${index + 1}`} value={row.sourceId} onChange={(event) => updateRow(index, "sourceId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "preparationId")} aria-label={`preparation_id ${index + 1}`} value={row.preparationId} onChange={(event) => updateRow(index, "preparationId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "cultureId")} aria-label={`culture_id ${index + 1}`} value={row.cultureId} onChange={(event) => updateRow(index, "cultureId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "plateId")} aria-label={`plate_id ${index + 1}`} value={row.plateId} onChange={(event) => updateRow(index, "plateId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "wellId")} aria-label={`well_id ${index + 1}`} value={row.wellId} onChange={(event) => updateRow(index, "wellId", event.target.value)} /></td>
                        <td><select {...rowProps(index, "factorLevel")} aria-label={`${dynamicFactorColumn} ${index + 1}`} value={row.factorLevel} onChange={(event) => updateRow(index, "factorLevel", event.target.value)}><option value="">{text(language, "Non riportato", "Not reported")}</option>{[...new Set([draft.levelA, draft.levelB, row.factorLevel].filter(Boolean))].map((level) => <option key={level} value={level}>{level}</option>)}</select></td>
                        <td><input {...rowProps(index, "batchId")} aria-label={`batch_id ${index + 1}`} value={row.batchId} onChange={(event) => updateRow(index, "batchId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "dayId")} aria-label={`day_id ${index + 1}`} value={row.dayId} onChange={(event) => updateRow(index, "dayId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "operatorId")} aria-label={`operator_id ${index + 1}`} value={row.operatorId} onChange={(event) => updateRow(index, "operatorId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "incubatorId")} aria-label={`incubator_id ${index + 1}`} value={row.incubatorId} onChange={(event) => updateRow(index, "incubatorId", event.target.value)} /></td>
                        <td><input {...rowProps(index, "timepoint")} aria-label={`timepoint ${index + 1}`} value={row.timepoint} onChange={(event) => updateRow(index, "timepoint", event.target.value)} /></td>
                        <td><input {...rowProps(index, "endpointId")} aria-label={`endpoint_id ${index + 1}`} value={row.endpointId} onChange={(event) => updateRow(index, "endpointId", event.target.value)} /></td>
                        <td><select {...rowProps(index, "lifecycleStatus")} aria-label={`lifecycle_status ${index + 1}`} value={row.lifecycleStatus} onChange={(event) => updateRow(index, "lifecycleStatus", event.target.value as LifecycleStatus)}>{LIFECYCLE_STATUSES.map((status) => <option key={status}>{status}</option>)}</select></td>
                        <td><input {...rowProps(index, "exclusionReason")} aria-label={`exclusion_reason ${index + 1}`} value={row.exclusionReason} disabled={row.lifecycleStatus !== "excluded"} placeholder="post-treatment | autore: …" onChange={(event) => updateRow(index, "exclusionReason", event.target.value)} /></td>
                        <td><input {...rowProps(index, "fileRef")} aria-label={`file_ref ${index + 1}`} value={row.fileRef} onChange={(event) => updateRow(index, "fileRef", event.target.value)} /></td>
            ...
            <div className="d0-review">
              ...
              {compileResult && (
                <section className="d0-result-section" aria-labelledby="canonical-d0-heading">
                  <div className="d0-subheading"><div><span className="eyebrow">{`${D0_RULESET_ID}@${D0_RULESET_VERSION}`} · API</span><h3 id="canonical-d0-heading">{text(language, "Esito canonico del compilatore", "Canonical compiler result")}</h3></div><span className={`rule-outcome outcome-${compileResult.ready_for_handoff ? "fired" : "abstained"}`}>{compileResult.ready_for_handoff ? "READY" : "WITHHELD"}</span></div>
              ...
              <PseudoreplicationRiskCard rows={rows} draft={draft} language={language} />

              <div className="d0-proof-grid">
                <section className="d0-result-section" aria-labelledby="proof-heading">
                  <div className="d0-subheading"><div><span className="eyebrow">{`client preview · ${D0_RULESET_ID}@${D0_RULESET_VERSION}`}</span><h3 id="proof-heading">{`Proof trace · ${D0_PREVIEW_RULE}`}</h3></div><span className={`rule-outcome outcome-${previewDeterminability === "DETERMINATE" ? "fired" : "abstained"}`}>{previewDeterminability === "DETERMINATE" ? "FIRED" : "ABSTAINED"}</span></div>
