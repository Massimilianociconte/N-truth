...
def main() -> int:
    ...
    if instance.get("training_eligible") is not False:
        ...
        return 1
    if instance.get("evaluation_eligible") is True and instance.get(
        "annotation_phase"
    ) == "internal_trial_3_5":
        print(
    ...
    if not instance.get("guideline_version") or not instance.get("schema_version"):
        ...
        return 1
    if instance.get("bundle_role") == "INTERNAL_REALITY_CHECK" and instance.get(
        "review_status"
    ) is None:
    ...
    if instance.get("reality_check_class") == "PROTOCOL_DRY_RUN":
        out["note"] = (
            "PROTOCOL_DRY_RUN: protocol stress-test only; not real anchor"
        )
