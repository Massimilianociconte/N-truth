#!/usr/bin/env python3
"""Qualificazione runtime MiniCPM5-2B MLX → PARTIALLY_VERIFIED (non scientifica).

Esegue i gate runtime su host locale (Apple Silicon / mlx-lm):
  1. verifica pesi e revision
  2. fingerprint artefatto
  3. load backend MiniCPM
  4. chat template (ChatML, enable_thinking=false) + special token / stop
...
Uso:
  uv run python scripts/models/qualify_minicpm_runtime.py
  uv run python scripts/models/qualify_minicpm_runtime.py --apply-transition
"""
...
PROFILE = REPO / "models" / "configs" / "minicpm5-2b-mlx-qlora.json"
EVIDENCE_DIR = REPO / "models" / "ntruth-minicpm-2b" / "manifests"
EXPECTED_SHA = "c207798696a4a454e7ac211b25227625466c693335941cee8904fb922f295cc1"
EXPECTED_BYTES = 1_416_035_216
MLX_REV = "8a9ad7539ac86281d0ac2b017ba04a5de53fe9a3"
CANONICAL_REV = "12a3808a956f869c767195e9266b59c4d21d92e2"
...
def build_fingerprint(
    *,
    ...
    raw = {
        ...
        "quantization": "4bit_mlx_official",
        "backend": "mlx-lm",
    ...
    fingerprint["mlx_repository"] = "openbmb/MiniCPM5-2B-MLX"
    fingerprint["mlx_kind"] = "official_vendor_mlx_openbmb"
    fingerprint["local_path"] = "models/local/minicpm5-2b-4bit"
...
def run_qualification() -> dict[str, Any]:
    ...
    from ntruth.model_backends.minicpm import chat_template_fingerprint
    ...
    # Chat template + special tokens (MiniCPM5 ChatML, thinking disabled).
    try:
        messages = [
        ...
        has_roles = (
            "<|im_start|>" in rendered
            and "<|im_end|>" in rendered
            and "system" in rendered
            and "user" in rendered
            and "assistant" in rendered
        )
        no_thinking = "<think>" not in rendered.lower() or "<think>\n\n</think>" in rendered
        ...
        checks.append(
            _check(
                ...
                has_roles and no_thinking and stop_ok,
                (
                    f"roles={has_roles} no_thinking_trace={no_thinking} "
    ...
    skeleton = {
        ...
        "provenance": {
            ...
            "stage": "candidate_graph_set",
            "authority": "model",
            "producer": "minicpm-runtime-qualification",
    ...
    try:
        ...
        has_fields = any(
            ...
            for name in ("determinability", "verdict", "n", "final_n", "independent_n")
        )
        if has_fields:
            checks[-1] = _check(
                "parser_cannot_emit_n_or_verdicts",
    ...
    evidence["summary"] = (
        "Il codice e migrato a MiniCPM5-2B (ADR-0019). "
        + (
            "Runtime PARTIALLY_VERIFIED candidate per questo fingerprint MLX ufficiale."
            if all_ok
...
def apply_registry_transition(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    model = payload["models"]["openbmb/MiniCPM5-2B"]
    ...
    record = append_qualification_transition(
        ...
        rationale=(
            "MiniCPM5-2B official MLX 4-bit weights verified; E2E inference, ChatML "
            "template with thinking disabled, stop tokens, candidate-only schema path, "
            "load/unload, and initial M5 micro-benchmark recorded for this exact "
            "artifact fingerprint. Scientific validation remains NOT_STARTED."
        ),
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    payload["models"]["openbmb/MiniCPM5-2B"]["runtime_qualification_status"] = loaded[
        "qualification"
    ]["runtime_qualification_status"]
    payload["models"]["openbmb/MiniCPM5-2B"]["verification_status"] = loaded["qualification"][
        "runtime_qualification_status"
    ]
