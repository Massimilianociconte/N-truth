...
class V8AdequacyVerificationError(ValueError):
    """Adequacy may consume only a fully verified canonical dependency claim set."""

    def __init__(self, report: object) -> None:
        self.report = report
        super().__init__("design adequacy evaluation requires an exactly verified DerivedClaimSet")
...
def evaluate_design_adequacy(
    ...
    *,
    ...
    from ntruth.verifier.v8 import verify_v8_derived_claim_set

    verification = verify_v8_derived_claim_set(
        request,
        claims,
    ...
    return (
        DesignAdequacyEvaluation(
            ...
            theory_id=pin.theory_id,
            theory_version=pin.theory_version,
            theory_checksum=pin.theory_checksum,
            theory_clause_id=rule.theory_clause_id,
            theory_clause_version=pin.theory_clause_version,
            rule_id=rule.rule_id,
            rule_version=rule.rule_version,
            rule_checksum=pin.rule_checksum,
            implementation_artifact_id=pin.implementation_artifact_id,
            implementation_artifact_version=pin.implementation_artifact_version,
            implementation_artifact_checksum=pin.implementation_artifact_checksum,
...
__all__ = ["V8AdequacyVerificationError", "evaluate_design_adequacy"]
