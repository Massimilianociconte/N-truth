...
def write_all(
    ...
    *,
    ...
    report = _reconcile_public_report(report)
...
def _reconcile_public_report(report: Report) -> Report:
    """Riconcilia il report pubblico con lo stato live dei blocchi.

    Il boundary di scrittura non si affida ai dati in cache: riverifica ogni
    blocco, riproietta gli output illeciti, rigenera compilation, valutazioni,
    positive output e summary dallo stato corrente e scarta qualsiasi dato
    stale (alert, n, metodi) che non sia piu autorizzato dal blocco live.
    """
        ...
        warning_codes = tuple(
            dict.fromkeys(
                (
                    ...
                    *(item.code for item in public_verification.warnings),
                    *(cached_verification.warning_codes if cached_verification is not None else ()),
                )
            )
        )
