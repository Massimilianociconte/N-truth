    ...
    *,
    ...
    for node_type, facts in grouped.items():
        ...
        if len(totals) == 1:
            ...
            # Due conteggi diversi con esclusioni riportate per lo stesso tipo di
            # unita sono la differenza attesa tra allocato e analizzato, non una
            # contraddizione (ARRIVE 2.0). Un processo di esclusione che non
            # dichiara il tipo resta sulla via conservativa della contraddizione.
            count = min(totals)
            ...
            acc.questions.append(
                Question(
                    id=stable_id(
                        ...
                        "contrast-truncated",
                        factor.name,
                        *(value for pair in dropped for value in pair),
                    ),
                    text=(
