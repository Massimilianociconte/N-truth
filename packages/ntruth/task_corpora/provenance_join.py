...
def normalize_caption(text: str) -> str:
    ...
    No case folding, no punctuation changes: the complete original text must
...
def classify_containment_dois(local_key: str, containing_dois: frozenset[str]) -> JoinDecision:
    ...
    if len(containing_dois) == 1:
        return JoinDecision(
            "TIER2_SINGLE_DOI_ARTICLE", "ARTICLE", article_doi=next(iter(containing_dois))
        )
    if len(containing_dois) > 1:
        return JoinDecision("UNMATCHED_MULTI_DOI", "UNKNOWN")
    return JoinDecision("UNMATCHED_NO_EVIDENCE", "UNKNOWN")
