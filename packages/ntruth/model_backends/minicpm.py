"""Backend MiniCPM5-2B via MLX-LM (primario provvisorio Train A, ADR-0019).

Checkpoint canonico: ``openbmb/MiniCPM5-2B`` (Apache-2.0).
Bootstrap Apple Silicon: distribuzione MLX 4-bit **ufficiale del vendor**
``openbmb/MiniCPM5-2B-MLX`` (non una conversione community).

Architettura ``LlamaForCausalLM`` standard: nessun kernel custom, nessun fork
del model-code. Il chat template MiniCPM5 e in stile ChatML
(``<|im_start|>``/``<|im_end|>``) con flag ``enable_thinking``: N-Truth rende
sempre con ``enable_thinking=False`` (blocco think vuoto, JSON diretto).
Thinking attivo romperebbe il parsing JSON e inietterebbe testo non-evidence
nei candidate facts: percio un tokenizer che rifiuta il flag fallisce
fail-closed invece di ricadere in thinking silenzioso.
...
class MiniCPMBackend(ModelBackend):
    """Implementazione MiniCPM5-2B via MLX-LM (Metal) — free decode only."""
    ...
    def __init__(
        ...
        *,
        ...
        max_tokens: int = 1024,
        canonical_model_id: str = MINICPM_CANONICAL_MODEL_ID,
        mlx_repo: str = MINICPM_MLX_REPO,
        revision: str = "",
        weight_sha256: str | None = None,
        context_window_tokens: int = MINICPM_CONFIGURED_MAX_CONTEXT_TOKENS,
    ) -> None:
    ...
    def model_metadata(self) -> ModelMetadata:
        return ModelMetadata(
            ...
            dtype="bf16_canonical_or_4bit_mlx_official",
            ...
            parameter_count=MINICPM_PARAMETER_COUNT,
            local_path=str(self.model_path),
            notes=(
                "Provisional primary MiniCPM5-2B backend (ADR-0019); not operational default.",
                "Qualification status is not stored in this module.",
                "Candidate facts only; rules engine owns scientific consequences.",
                f"Official vendor MLX 4-bit: {self.mlx_repo}",
                f"Chat template {MINICPM_CHAT_TEMPLATE}: always enable_thinking=false.",
    ...
    def load(self) -> None:
        ...
        if not self.model_path.exists():
            raise ComponentLoadError(
                f"pesi MiniCPM assenti: {self.model_path} "
                "(eseguire scripts/models/acquire_minicpm.py fuori da Git)"
            )
        ...
        try:
            ...
            raise ComponentLoadError(f"caricamento MiniCPM/MLX fallito: {exc}") from exc
    ...
    def tokenize(self, text: str) -> list[int]:
        ...
        if not callable(encode):
            raise MiniCPMBackendError("tokenizer senza encode")
    ...
    def apply_chat_template(
        ...
        *,
        ...
        try:
            ...
            raise MiniCPMBackendError(
                "tokenizer non supporta enable_thinking=false: thinking mode "
                f"non ammesso per output JSON candidate-only ({exc})"
            ) from exc
    ...
    def generate_structured(self, request: GenerationRequest) -> GenerationResult:
        ...
        if want_constrained:
            raise ConstrainedDecodingUnavailable(
                "constrained decoding non incluso nel backend MiniCPM; "
        ...
        if max_tokens <= 0:
            raise MiniCPMBackendError("max_tokens deve essere > 0 (esplicito)")
