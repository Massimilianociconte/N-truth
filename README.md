...
On Apple Silicon, also run `uv run python scripts/smoke_release.py --include-ml`;
it installs the optional ML extra from wheel and sdist without downloading model
weights.
...
silently “repair” those authoritative source examples. A green suite demonstrates the
tested software contracts; it does not demonstrate accuracy on real documents, human
agreement or external validity.
