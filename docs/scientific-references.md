...
The 32 current rules in `rulesets/ntruth-core-0.3.0.json` cite stable identifiers `R01` through
...
uv run python -c 'import json; p=json.load(open("rulesets/ntruth-core-0.3.0.json")); print({r["rule_id"]: r["references"] for r in p["rules"]})'
```
