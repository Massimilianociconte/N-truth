...
| SRR-0006 | 2026-08-07 | §7.9, App. P.1 | Denominazione del Canonical Count Registry | §7.9 usa `*_unit_count`/`diagnostic_effective_n`; Appendice P.1 e gli esempi usano `planned_n`/`effective_n_diagnostic`. Fail-closed: entrambi i vocabolari sono accettati in lettura; la serializzazione usa i nomi §7.9; `independent_n`, `planned_n`, `effective_n` restano alias deprecati (pattern `INDETERMINATE`) | PENDING_REVIEW | — |
