...
| SRR-0017 | 2026-08-07 | §7.15 E, App. Y.1, §2.4, NFR-26 | Token positivi di estimand-support e conseguenze interference | Il PRD determina solo il token negativo `UNSUPPORTED_OR_UNSPECIFIED` (documented interference senza exposure mapping/estimand appropriato); nessun token positivo di supporto estimand è definito. Fail-closed (`derive_estimand_support`): anche con interference DOCUMENTED ed exposure mapping PRESENT l'estimand resta `UNKNOWN` — il motore non dichiara mai supporto estimand positivo senza revisione. Le classi di conseguenza Y.1 sono emesse esclusivamente come claim/finding di exposure/estimand/scope (tipo `DESIGN_ADEQUACY_FINDING`, asse `INTERFERENCE`); l'identità e il conteggio dell'EU sono immutabili rispetto all'interference; il silenzio (`interference_status=UNKNOWN`) non produce alcun finding (NFR-26) e ogni stato dichiarato diverso da UNKNOWN richiede evidenza propria | PENDING_REVIEW | — |
| SRR-0018 | 2026-08-07 | §7.7, App. P.4 | Conversione dell'alias legacy `allocation_timing` in timing event-referenced | Il PRD depreca il timing libero ma non determina una tabella di conversione. Fail-closed (`legacy_timing_relation`, `Factor.timing_relation`): solo marcatori univoci case-insensitive (`before`/`after`/`same event`/`overlap(s)`/`unknown`) sono convertiti nelle relazioni §7.7; ogni altra stringa (inclusi ordini di frase come "pool then split") e ogni catena senza `relative_timing` tipizzato restano `UNKNOWN`; l'ordine della frase non e' mai assunto e nessun evento di riferimento viene inventato | PENDING_REVIEW | — |
